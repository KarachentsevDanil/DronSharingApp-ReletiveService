  ...
  <div class="panel panel-flat facility-contact">
    <div class="panel-body">
      <h6 class="no-margin-top">
        <a>{{ residentDetails.FacilityName }}</a>
      </h6>
    </div>
    <div class="facility-map">
      <iframe width="100%" height="200" frameborder="0" style="border:0" :src="mapUrl"></iframe>
    </div>
    <div class="panel-body">
      <div class="form-group">
        <label class="control-label no-margin text-semibold">
          <i class="icon-phone2"></i> Phone:
        </label>
        <div class="pull-right">{{ residentDetails.FacilityPhone }}</div>
      </div>
      <div class="form-group">
        <label class="control-label no-margin text-semibold">
          <i class="icon-envelop"></i> Email:
        </label>
        <div class="pull-right">{{ residentDetails.FacilityEmail }}</div>
      </div>
    </div>
    <div class="panel-body text-center alpha-grey">
      ...
      <ul class="timer-weekdays mb-10">
        ...
        >
          <a
            :class="[ day.isActive?'label-danger':'label-default', 'label' ]"
            @click="activateDay(index)"
          >{{ day.day }}</a>
        </li>
      </ul>

      <ul class="timer mb-10">{{ workingHours }}</ul>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      mapUrl:
        "https://www.google.com/maps/embed/v1/place?key=AIzaSyBP2LMWhQWgru1NtX5o20oScy5g0twDWH8&q=Ukraine"
    };
  },
  props: {
    residentDetails: {
